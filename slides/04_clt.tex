...
\end{align*}
を示せばよい。$Y$は分散を持つので$H_Y$は0で二回微分できる。テイラーの定理($H_Y$は$\mathbb{R}\to\mathbb{C}$の関数だけど)より
\begin{align*}
...
\stackrel{N\to\infty}{\longrightarrow} -\frac{t^2}2
\end{align*}
\end{frame}
...
\begin{frame}{キュムラントを使った証明}
\begin{align*}
H_{X+a}(t) &= \log\expt{\mathrm{e}^{it(X+a)}} = \log\left(\expt{\mathrm{e}^{itX}}\cdot \mathrm{e}^{ita}\right) = \emm{H_X(t) + iat}\\
H_{aX}(t) &= \log\expt{\mathrm{e}^{it(aX)}} = \emm{H_X(at)}\\
H_{X+Y}(t) &= \log\expt{\mathrm{e}^{it(X+Y)}} =\log\left(\expt{\mathrm{e}^{itX}}\expt{\mathrm{e}^{itY}}\right) = \emm{H_X(t) + H_Y(t)}
\end{align*}
$X\sim N(\mu,\sigma^2)$について
\begin{align*}
H_X(t) &= i\mu t - \frac{\sigma^2t^2}2
\end{align*}
より、
\begin{align*}
H_{X\emm{+a}}(t) &= i(\mu\emm{+a}) t - \frac{\sigma^2t^2}2\qquad\text{\small ($N(\mu+a,\sigma^2)$のキュムラント母関数)}\\
H_{\emm{a}X}(t) &= i\mu \emm{a}t - \frac{\emm{a^2}\sigma^2t^2}2\qquad\text{\small ($N(a\mu,a^2\sigma^2)$のキュムラント母関数)}\\
H_{X_1\emm{+}X_2}(t) &= i(\mu_1\emm{+}\mu_2)t - \frac{(\sigma_1^2\emm{+}\sigma_2^2)t^2}2\qquad\text{\small ($N(\mu_1+\mu_2,\sigma_1^2+\sigma_2^2)$のキュムラント母関数)}
\end{align*}
\end{frame}
