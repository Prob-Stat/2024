...
\begin{frame}{分散共分散行列}
\small
確率変数$Z_1,\dotsc,Z_n$の\emm{分散共分散行列}(もしくは\emm{共分散行列})$\symbf{\Sigma}\in\mathbb{R}^{n\times n}$は
...
分散共分散行列は
\begin{align*}
\symbf{\Sigma}&=\expt{\symbf{Z} \symbf{Z}^T} - \expt{\symbf{Z}}\expt{\symbf{Z}}^T
\end{align*}
である。
\end{frame}
...
\begin{align*}
\symbf{\Sigma}&=\expt{\symbf{AZ} \symbf{AZ}^T} - \expt{\symbf{AZ}}\expt{\symbf{AZ}}^T\\
