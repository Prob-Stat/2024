...
$x\in\mathrm{Range}(f)$について

\begin{align*}
\Pr(f(X) \le x)&= \Pr\left(X\le f^{-1}(x)\right)\\
&=\int_{-\infty}^{f^{-1}(x)} p(t) \mathrm{d}t\\
&=\int_{-\infty}^{x} p\left(f^{-1}(u)\right) \frac1{f'(f^{-1}(u))}\mathrm{d}u\qquad(u = f(t))
\end{align*}
...
とすると、$X^2$の確率密度関数は
\begin{align*}
\Pr(X^2\le x) &= \Pr(X\in [-\sqrt{x},\,\sqrt{x}])\\
&= \int_{-\sqrt{x}}^{\sqrt{x}} p(x)\mathrm{d}x\\
\end{align*}

\begin{align*}
\Pr(X^2\le x) &= \Pr(X\le \sqrt{x})
\end{align*}
\begin{align*}
\frac1{2\sqrt{x}}p(\sqrt{x})
&=
