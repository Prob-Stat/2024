...
ある$v\in\mathbb{C}^{N+1}\setminus\{0\}$について$V(x_0,\dotsc,x_N) v = 0$のとき、高々$N$次の多項式を$P(x):=\sum_{k=0}^N v_k x^k$と定義する。この多項式は少なくとも$N+1$個の根$x_0,\dotsc,x_N$を持つ。高々$N$次の多項式は相異なる根を高々$N$個持つのでこれは矛盾。
\end{proof}
\end{frame}
