...
\vspace{1em}
$f$は凸なので、$yx-f(x)$は$x$の関数として凹(上に凸)となる。
このとき、
\begin{itemize}
\setlength{\itemsep}{1em}
\item $f'(x^*) = y$ を満たす$x^*\in J$が存在するとき、$f^*(y) = yx^* - f(x^*)$である。
$f'$が単調非減少であることから、$x^*$は$y$について単調非減少である。
\item $f'(x^*) = y$ を満たす$x^*\in J$が存在しないとき、$\sup$は区間の端への極限で得られる。
\end{itemize}
\end{frame}
...
$f'(x^*) = y$より、$y=f'(0)$のとき、$x^*=0$である。特に$f(0)=0$のとき以下の図のようになる。
...
$x^*$は$y=f'(0)$のとき0で$y>f'(0)$のとき、$x^*>0$、$y<f'(0)$のとき$x^*<0)$。
\end{frame}
