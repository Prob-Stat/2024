...
\begin{align*}
\Pr(X\ge a)&\le \mathrm{e}^{-I_+(a)}\\
\Pr(X\le a)&\le \mathrm{e}^{-I_-(a)}.
\end{align*}
\end{theorem}
\end{frame}
...
% \mathrm{e}^{N(K_X(t) - at)}
\Pr\left(\frac1N\sum_{k=1}^N X_k\ge a\right) &\le \mathrm{e}^{-I_+(a)N}\\
\Pr\left(\frac1N\sum_{k=1}^N X_k\le a\right) &\le \mathrm{e}^{-I_-(a)N}.
\end{align*}
\end{lemma}
\begin{proof}
\small
