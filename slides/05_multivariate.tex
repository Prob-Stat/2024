...
$p_{X_1,\dotsc, X_n}\colon \mathbb{R}^n\to\mathbb{R}$を
確率変数$X_1,\dotsc, X_n$の\emm{確率密度関数}とすると、
任意の$A\subseteq\mathbb{R}^n$について
%$\stackrel{\mathrm{def}}{\iff}$
\begin{align*}
\Pr([X_1,\,X_2,\dotsc, X_n]\in A)&=
...
\int_{A} p_{X_1,\dotsc, X_n}(x)\mathrm{d}x.
\end{align*}
\end{block}
%\end{definition}
...
$p_{X_1,X_2}(x_1,x_2)$は$x_1\ne x_2$のとき0としてよいが、$p(x,x)$をどのように定義しても$\int_{\mathbb{R}^2}p(x_1,x_2)\mathrm{d}x_1\mathrm{d}x_2$は0になる。
\end{example}
...
\end{align*}
ここで$\langle t,\, X\rangle := \emm{\sum_{k=1}^n t_k X_k}$である。
