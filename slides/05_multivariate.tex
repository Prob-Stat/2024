...
&=
\int_{\mathbb{R}^n} \frac1{\sqrt{(2\pi)^n\det(D)}} \mathrm{e}^{-\frac12 z^T z}\frac1{|\det(\sqrt{D^{-1}})|}\mathrm{d}z\qquad (z=\sqrt{D^{-1}}y)\\
&=
...
\begin{corollary}
$n$次元の確率変数$\symbf{Z}\sim N(\mu, \Sigma)$について、
$[Z_1,\dotsc,Z_{n-1}]\sim N(\mu', \Sigma')$.
%$Z_k\sim N(\mu_k, \Sigma_{k,k})$.
\end{corollary}
...
\small
$[X, Y]\sim N\left(\begin{bmatrix}0\\0\end{bmatrix},\,\begin{bmatrix}1&0\\0&1\end{bmatrix}\right)$について極座標表示$[R, \Theta]$を考える。
つまり、
\begin{align*}
...
&\Pr([R, \Theta]\in A)\\
&\qquad=
\int_{[R(x,y),\,\Theta(x,y)]\in A} \frac1{2\pi}\mathrm{e}^{-\frac12(x^2+y^2)}\mathrm{d}x\mathrm{d}y\\
&\qquad=
\int_{[R(x,y),\,\Theta(x,y)]\in A} \frac1{2\pi}\cdot\mathrm{e}^{-\frac12r^2}r\mathrm{d}r\mathrm{d}\theta\\
&\qquad=
