...
\item $x$: データ点
%\item $\theta$: パラメータ、母数
\item $s$: パラメータ(今日は離散の場合を考える。連続のときは$\theta$という記号を使うことが多い)
\item $\Pr(S = s)$: 事前確率
...
\begin{align*}
\Pr\left(E(\symbf{X}) \ne S\right)
&= \Pr\left(p(S) p(\symbf{X}\mid S) < p(\overline{S}) p(\symbf{X}\mid \overline{S})\right)\\
&= \Pr\left(\emm{\sum_k \log\frac{p(X_k\mid S)}{p(X_k\mid \overline{S})}} < \log\frac{p(\overline{S})}{p(S)}\right).
\end{align*}
\end{frame}
...
\Pr\left(\emm{\sum_k \log\frac{p(X_k\mid S)}{p(X_k\mid \overline{S})}} < \log\frac{p(\overline{S})}{p(S)}\right)
&=
\lambda \Pr\left(\emm{\sum_k \log\frac{p(X_k\mid 0)}{p(X_k\mid 1)}} < \log\frac{p(1)}{p(0)}\mid S=0\right)\\
&+
(1-\lambda) \Pr\left(\emm{\sum_k \log\frac{p(X_k\mid 1)}{p(X_k\mid 0)}} < \log\frac{p(0)}{p(1)}\mid S=1\right)
\end{align*}
