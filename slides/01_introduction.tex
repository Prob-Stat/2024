...
\begin{enumerate}
\item $P(\Omega)=1$.
\item $\forall A, B\subseteq\Omega$, $A\cup B=\emptyset\implies P(A\cup B)=P(A)+P(B)$\\ (\emm{有限加法性}).
\end{enumerate}
...
\begin{enumerate}
\item $P(\Omega)=1$.
\item {\color{lightgray}$\forall A, B\subseteq\Omega$, $A\cup B=\emptyset\implies P(A\cup B)=P(A)+P(B)$\\ (有限加法性).}\\
\item[2'.] $\forall (A_n\subseteq\Omega)_{n=1,\dotsc}$, $\forall i\ne j,\, A_i\cup A_j=\emptyset\implies P\left(\bigcup_{n=1}^\infty A_n\right)=\sum_{n=1}^\infty P(A_n)$\\ (\emm{完全加法性, $\sigma$-加法性}).
\end{enumerate}
...
\begin{enumerate}
\item $P(\Omega)=1$.
\item $\forall A, B\in\mathcal{F}$, $A\cup B=\emptyset\implies P(A\cup B)=P(A)+P(B)$\\ (有限加法性).
\item[2'.] $\forall (A_n\in\mathcal{F})_{n=1,\dotsc}$, $\forall i\ne j,\, A_i\cup A_j=\emptyset\implies P\left(\bigcup_{n=1}^\infty A_n\right)=\sum_{n=1}^\infty P(A_n)$\\ (完全加法性, $\sigma$-加法性).
\end{enumerate}
\end{frame}
...
\begin{align*}
\Pr(X\ge a) &\coloneq \Pr(X\in \{x\in\mathbb{R}\mid x\ge a\})
\end{align*}
...
\vspace{2em}
特に$X$の象が高々可算のとき、$X$を\emm{離散型確率変数}といい、$f_X(a)\coloneq\Pr(X=a)$を\emm{確率質量関数}という。
離散型確率変数でない確率変数を\emm{連続型確率変数}という。

\vspace{1em}
一般に$F_X(a)\coloneq \Pr(X\le a)$を\emm{累積分布関数}という。
\end{frame}
