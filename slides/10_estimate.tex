...
\begin{align*}
\Pr(X_1=x_1,\,X_2=x_2) &=\frac{\lambda^{x_1+x_2}}{x_1!x_2!}\mathrm{e}^{-2\lambda}
\end{align*}
に従っているとき、$T(x_1,x_2)=\emm{x_1+x_2}$は\emm{$\lambda$の十分統計量}である。
実際、
%
\begin{align*}
\Pr(T(X_1,\,X_2)=t) &= \sum_{x=0}^t\Pr(X_1=x,\, X_2=t-x)\\
&=\sum_{x=0}^t \frac{\lambda^{t}}{x!(t-x)!}\mathrm{e}^{-2\lambda}\\
&=\frac1{t!}(2\lambda)^t\mathrm{e}^{-2\lambda}.
\end{align*}
よって$x_1+x_2=t$のとき、
\begin{align*}
\Pr(X_1=x_1,\,X_2=x_2\mid T(X_1,\,X_2) = t) &=\frac{\lambda^{x_1+x_2}}{x_1!x_2!}\mathrm{e}^{-2\lambda}
t!(2\lambda)^{-t}\mathrm{e}^{2\lambda}\\
&=\binom{t}{x_1}2^{-t}.
\end{align*}
$\lambda$に依存してない(二項分布$\mathrm{Binom}(t, \frac12)$)。
...
つまり
\begin{align*}
\Pr_\theta(X\in A) &= \Pr_\theta(-X\in A)
\end{align*}
