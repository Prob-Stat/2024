...
第一種誤り確率は$\alpha_E\coloneq\expt{E(X)\mid S= 0}$、
...
\begin{align*}
E_{\mathrm{MAP}}(x)\coloneq\emm{\argmax_{\theta\in\Theta} p(\theta\mid x)}
\end{align*}
...
\begin{align*}
E_{\mathrm{ML}}(x)\coloneq\emm{\argmax_{\theta\in\Theta} p(x\mid \theta)}
\end{align*}
平均誤り確率については何も言えない。事前分布を知らなくても使える。

\vspace{1.5em}
事前分布が一様分布のとき($\Theta$が有界のときしか定義できない)、最尤推定と最大事後確率推定は等しい。
\end{frame}
...
\end{align*}
\begin{align*}
\log\frac{p(\symbf{x}\mid \theta_0)}{p(\symbf{x}\mid \theta_1)} &= N\frac{\sigma_1^2-\sigma_0^2}{2\sigma^2} + \frac{\theta_0-\theta_1}{\sigma^2}\sum_{k=1}^N x_k
\end{align*}
